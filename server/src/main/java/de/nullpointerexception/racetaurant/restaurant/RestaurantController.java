...
public class RestaurantController {
	...
	List<Restaurant> filtered(
			...
			@RequestParam(required = false) String timeStop,
		...
		if (price != null && !RestaurantValidator.validatePrice(price)){
			throw new InvalidQueryParameterException("Price has to be the number 0 (cheap), 1 (medium) or 2 (expensive)");
		}
		...
		return repository.findFiltered(start, limit, priceCategory, latitude, longitude, radius, cuisineArray, ratingMin, ratingMax, timeStart, timeStop, persons, order, asc);
	}

	@GetMapping("/restaurants/{id}")
	Restaurant one(@PathVariable String id) {
	...
	@ControllerAdvice
	class InvalidQueryParameterAdvice {
		@ResponseBody
		@ExceptionHandler(InvalidQueryParameterException.class)
		@ResponseStatus(HttpStatus.BAD_REQUEST)
		String invalidQueryParameterHandler(InvalidQueryParameterException ex) {
			return ex.getMessage();
		}
	}
